          ...
          <div className="grid grid-cols-3 border-t">
            ...
            <div className="p-4 flex items-center justify-center">
              ...
              <>
                ...
                <Button
                  onClick={() => {
                    router.push(`/events/${event.slug}/${selectedEventScheduleId}/select-ticket`)
                  }}
                  className="bg-black text-white rounded-md py-2 px-3 md:px-6 hover:bg-gray-800 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                  disabled={!selectedEventScheduleId}
                >
                  {t('event.selectTicket')}
                </Button>

                {/* <SelectAndPurchaseTicketModal
                  isOpen={isOpen}
                  onCloseModal={() => setIsOpen(false)}
                  event={event}
                  unavailableSeats={unavailableSeats}
                  eventScheduleId={selectedEventScheduleId}
                /> */}
              </>
            </div>
          </div>
