...
interface Props {
  typeFilter: 'booked' | 'canceled' | 'pending_payment' | 'gifted'
  setTypeFilter: (val: 'booked' | 'canceled' | 'pending_payment' | 'gifted') => void
  t: (key: string) => string
}
...
export const TicketTypeFilterTab: React.FC<Props> = ({ typeFilter, setTypeFilter, t }) => (
  <div className="flex gap-6 text-sm font-medium mb-4">
    {(['booked', 'canceled', 'pending_payment', 'gifted'] as const).map((filter) => (
      <button
        key={filter}
