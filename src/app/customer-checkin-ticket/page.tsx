...
const CheckInResult: React.FC<CheckInResultProps> = ({ data, confirmed, onReset, onConfirm }) => {
  ...
  const bulkCheckIn = async () => {
    if (!selectedCodes.length) {
      toast.toast({ variant: 'destructive', title: 'No selection', description: 'Select at least one sister ticket.' })
      return
    }
    try {
      ...
      toast.toast({ title: 'Bulk Check-in', description: json.message || 'Checked in selected tickets.' })
      setSelectedCodes([])
      setBulkMode('none')
    } catch {
      toast.toast({ variant: 'destructive', title: 'Error', description: 'Bulk check-in failed.' })
    }
  }

  const bulkMarkGiven = async () => {
    if (!selectedCodes.length) {
      toast.toast({ variant: 'destructive', title: 'No selection', description: 'Select at least one ticket.' })
      return
    }
    ...
    try {
      ...
      const json = await res.json()
      toast.toast({ title: 'Bulk Mark Given', description: json.message || 'Marked selected tickets as given.' })
      setSelectedCodes([])
      setBulkMode('none')
    } catch {
      toast.toast({ variant: 'destructive', title: 'Error', description: 'Bulk mark-as-given failed.' })
    }
  }
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center space-y-6 mb-6">
      ...
      >
        ...
        <div className="text-center space-y-4 bg-white p-6 rounded-lg">
          ...
          <div className="mt-4 flex flex-col space-y-2">
            {!data?.sisterTickets && (
              <Button onClick={onConfirm} className="w-full text-white" style={{ backgroundColor: zoneCategory?.color }}>
                Đã nhận vé vào cửa
              </Button>
            )}
            <Button onClick={onReset} className="w-full bg-white text-black" style={{ borderColor: zoneCategory?.color }}>
      ...
      <div className="w-full max-w-md bg-white p-4 rounded-lg shadow-sm">
        <h3 className="text-lg font-semibold mb-2">Sister Tickets</h3>
        {data?.sisterTickets && data?.sisterTickets.length > 0 && (
          <div className="w-full max-w-md flex gap-2 p-2 m-2">
            <Button variant="outline" onClick={() => setBulkMode('checkin')} className="flex-1">Bulk Check-In</Button>
            <Button variant="outline" onClick={() => setBulkMode('given')} className="flex-1">Bulk Mark Given</Button>
          </div>
        )}
        <ul className="space-y-2">
          ...
          {data?.sisterTickets && data?.sisterTickets.map(sister => (
            <li key={sister.ticketCode} className="flexitems-center rounded">
          ...
          {!data?.sisterTickets?.length && (
            <li className="p-2 text-sm text-gray-500">No sister tickets available.</li>
          )}
        </ul>
      </div>
      {/* Bulk action toggles & buttons */}
        ...
        {bulkMode === 'checkin' && <Button variant="secondary" onClick={bulkCheckIn} className="w-full max-w-md">Check In Selected</Button>}
        {bulkMode === 'given' && <Button variant="secondary" onClick={bulkMarkGiven} className="w-full max-w-md">Mark Selected as Given</Button>}
        {bulkMode !== 'none' && <Button variant="outline" onClick={() => setBulkMode('none')} className="w-full max-w-md ">Cancel Bulk</Button>}
...
export default function CustomerCheckInPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.toast({ title: 'Success', description: 'Ticket checked in successfully' })
      } else {
        const err: CheckInResponse = await response.json()
        toast.toast({ variant: 'destructive', title: 'Error', description: err.message })
      }
    } catch {
      toast.toast({ variant: 'destructive', title: 'Error', description: 'Failed to check in' })
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const handleTicketGiven = async () => {
    const adminId = window.prompt('Enter admin ID:')
    if (!adminId) return
    const confirm = window.confirm(`Confirm this ticket has been given by admin "${adminId}"?`)
    if (!confirm) return
    try {
      ...
      if (response.ok) {
        ...
        const res: CheckInResponse = await response.json()
        toast.toast({ title: 'Confirmed', description: res.message })
      } else {
        const err: CheckInResponse = await response.json()
        toast.toast({ variant: 'destructive', title: 'Error', description: err.message })
      }
    } catch {
      toast.toast({ variant: 'destructive', title: 'Error', description: 'Unexpected error' })
    }
  }

  if (checkedInData) {
    return (
      <CheckInResult data={checkedInData} confirmed={ticketGivenConfirmed} onReset={resetForm} onConfirm={handleTicketGiven} />
    )
  }
    ...
    <div className="min-h-screen flex items-center justify-center">
      <div className="w-full max-w-md p-6 space-y-6 bg-white rounded-lg shadow-lg">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <Input type="text" value={ticketCode} onChange={(e) => setTicketCode(e.target.value)} placeholder="Enter your ticket code" required />
